...
func NewStore(backend backend.Backend, config StoreConfig) (Store, error) {
	result := &store{
		...
		indexWorkingSet: newAuroraLiteGauge("working_set",
			"Approximately how much memory (kiB) are indices using?"),
		indexWorkingSetTime: newAuroraLiteGauge("working_set_time",
...
func newAuroraLiteGauge(name, help string) prometheus.Gauge {
	return prometheus.NewGauge(prometheus.GaugeOpts{
		Namespace: "aurora_lite",
