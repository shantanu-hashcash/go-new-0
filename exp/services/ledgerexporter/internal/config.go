...
type HcnetCoreConfig struct {
	NetworkPassphrase     string   `toml:"network_passphrase"`
	HistoryArchiveUrls    []string `toml:"history_archive_urls"`
	HcnetCoreBinaryPath string   `toml:"hcnet_core_binary_path"`
...
type Config struct {
	...
	HcnetCoreConfig HcnetCoreConfig `toml:"hcnet_core_config"`
...
func (config *Config) GenerateCaptiveCoreConfig() ledgerbackend.CaptiveCoreConfig {
	coreConfig := &config.HcnetCoreConfig

	// Look for hcnet-core binary in $PATH, if not supplied
	if coreConfig.HcnetCoreBinaryPath == "" {
		var err error
		coreConfig.HcnetCoreBinaryPath, err = exec.LookPath("hcnet-core")
		logFatalIf(err, "Failed to find hcnet-core binary")
	}
	...
	return ledgerbackend.CaptiveCoreConfig{
		BinaryPath:          coreConfig.HcnetCoreBinaryPath,
