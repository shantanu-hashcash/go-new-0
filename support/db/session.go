...
//
// tries to replace dbErr with aurora package error, returns a new error if the err is known.
...
func (s *Session) handleError(dbErr error, ctx context.Context) error {
	...
	case go_errors.Is(ctx.Err(), context.Canceled):
		// when aurora's context is cancelled by it's upstream api client,
	...
	case go_errors.Is(ctx.Err(), context.DeadlineExceeded):
		// when aurora's context times out(it's set to app connection-timeout),
