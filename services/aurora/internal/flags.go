...
func Flags() (*Config, support.ConfigOptions) {
	...
	var flags = support.ConfigOptions{
		...
		&support.ConfigOption{
			...
			Usage: "defines the timeout for when aurora will cancel all postgres queries connected to an HTTP request. The timeout is measured in seconds since the start of the HTTP request. Note, this timeout does not apply to POST /transactions. " +
				...
				"By default, client-query-timeout will be set to twice the connection-timeout. Setting client-query-timeout to 0 will disable the timeout which means that Aurora will never kill long running queries using the cancel request, however, " +
		...
		&support.ConfigOption{
			...
			Usage: "sets the limit on the maximum number of concurrent http requests, default is 1000, to disable the limit set to 0. " +
				"If Aurora receives a request which would exceed the limit of concurrent http requests, Aurora will respond with a 503 status code.",
			UsedInCommands: ApiServerCommands,
		},
		&support.ConfigOption{
